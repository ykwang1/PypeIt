...
def objfind(image, thismask, slit_left, slit_righ, inmask=None, fwhm=3.0, maxdev=2.0, ir_redux=False, spec_min_max=None,
    ...
    # For BinoSpec
    #flux_spec = extract_asymbox2(thisimg[2900:3940, :], left_asym[2900:3940, :], righ_asym[2900:3940, :], weight_image=totmask.astype(float)[2900:3940, :])
    #mask_spec = extract_asymbox2(totmask[2900:3940, :], left_asym[2900:3940, :], righ_asym[2900:3940, :], weight_image=totmask.astype(float)[2900:3940, :]) < 0.3
    flux_spec = extract_asymbox2(thisimg[3200:3600, :], left_asym[3200:3600, :], righ_asym[3200:3600, :], weight_image=totmask.astype(float)[3200:3600, :])
    mask_spec = extract_asymbox2(totmask[3200:3600, :], left_asym[3200:3600, :], righ_asym[3200:3600, :], weight_image=totmask.astype(float)[3200:3600, :]) < 0.3
